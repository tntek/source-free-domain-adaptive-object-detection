...
def inference_on_dataset(
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            # image = inputs[0]["image"].unsqueeze(0).cuda().float()  # 添加新维度
            # image_sizes = [image.shape[-2:]]  # 获取图像尺寸
            # images = ImageList(image, image_sizes)  # 转换为ImageList
            ...
            # ious = pairwise_iou(Boxes(proposal_boxes), Boxes(gt_boxes))
            # max_ious, _ = ious.max(dim=0)
            # covered_gt_boxes += (max_ious >= 0.5).sum().item()

            # visualize_proposals(cfg,images,gt_boxes,0,14,'rpn',test_metadata,'s')
