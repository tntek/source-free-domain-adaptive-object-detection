...
class ROIHeads(torch.nn.Module):
    ...
    def _sample_proposals_all(
        ...
        sampled_idxs = torch.cat([sampled_fg_idxs, sampled_bg_idxs], dim=0)
        # return sampled_idxs, gt_classes[sampled_idxs] ,gt_classes
        return sampled_fg_idxs, gt_classes[sampled_fg_idxs], sampled_bg_idxs, gt_classes[sampled_bg_idxs],gt_classes
    @torch.no_grad()
    def label_and_sample_proposals_withoutgt(
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            fg_index,fg_label,bg_index,bg_label ,label= self._sample_proposals_all(
                matched_idxs, matched_labels, targets_per_image.gt_classes
            )
            labels = self.label_proposals_from_iou(match_quality_matrix,0.5)
            if start==True:
                matched_labels_all = matched_labels.unsqueeze(0)
                start = False
            else:
                matched_labels_all = torch.cat((matched_labels_all,matched_labels.unsqueeze(0)),dim=0)
        ...
        return fg_index,fg_label,bg_index,bg_label ,label, labels
    
    def label_proposals_from_iou(self,match_quality_matrix, iou_threshold=0.5):
        # match_quality_matrix: shape [num_gt, num_props], dtype: torch.Tensor
        num_gt, num_props = match_quality_matrix.shape
        labels = -1 * torch.ones(num_props, dtype=torch.long, device=match_quality_matrix.device)

        max_ious, gt_indices = torch.max(match_quality_matrix, dim=0)  # 对每个proposal取最大IoU的gt
        labels[max_ious > iou_threshold] = gt_indices[max_ious > iou_threshold]
        
        return labels  # shape: [num_props]
