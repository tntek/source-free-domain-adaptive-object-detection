...
__all__ = ["student_sfda_RCNN_mt_voc"]
@META_ARCH_REGISTRY.register()
class student_sfda_RCNN_mt_voc(nn.Module):
    """
    ...
    def feat_kd_high_loss(self, feat, teacher_feat):
        ...
        return loss
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], cfg=None, model_teacher=None, t_features=None, t_proposals=None, t_results=None,epoch=None,weight=None,emaweight=None,move=None,cluster_step=None,temp=None,warm=None,mode="test"):

        if not self.training and mode == "test":
            return self.inference(batched_inputs)


        images = self.preprocess_image_weak(batched_inputs, mode)
        ...
        # results_weak, detector_losses_weak = self.roi_heads(images_weak, features_weak, proposals_weak,t_results)
        ...
        # visualize_proposals(cfg,batched_inputs,t_proposals,0,3,'rpn',MetadataCatalog.get(cfg.DATASETS.TRAIN[0]),'w')
        ...
        base_bg_index = torch.where(base_l == self.num_class)[0]
        base_bg_logtis_top2,_ = torch.topk(base_output[base_bg_index],k=2)

        base_bg_select = base_bg_index[torch.where(base_bg_logtis_top2[:,0] > 0.9)]
        ...
        dd_c ,labelset_c,all_labels= self.pro_obtain_label(t_cnn_pol,t_s_roih_logits[0],self.num_class + 1,prototype_pol,cluster_step,pro_index,fg_index,fg_labels,base_bg_select)

        # if len(pro_index)==8:
        #     losses["pro_const"] = self.pro_const_loss(s_cnn_pol,t_cnn_pol,self.prototype_pol)

        if epoch==1:
            dd ,labelset_m,label_clus= self.obtain_label(memory_fea,memory_logits,self.num_class + 1,2,fg_index,fg_labels,base_bg_select)
        ...
        if len(fg_index) >2 :
            if proposal_variance > 20:
                losses['cont'] = self.con_loss(s_cnn_pol[fg],all_labels[fg],0.07) * weight
            else:
                losses['cont'] = self.con_loss(s_cnn_pol,all_labels,0.07) * weight
                
        #     # losses['cont'] = self.con_loss(s_cnn_pol[fg_index],fg_labels,0.07) * 0.5
    ...
    def inference(
        ...
        if do_postprocess:
            ...
            return student_sfda_RCNN_mt_voc._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
    ...
    def obtain_label(self,all_fea,aff,K,step,fg_index,fg_labels,base_bg_select):#获取对应伪标签
    ...
    def pro_obtain_label(self,all_fea,aff,K,prototype,step,pro_index,fg_index,fg_labels,base_bg_select):#获取对应伪标签
        act=False
        if len(pro_index)==8:
            act = True
