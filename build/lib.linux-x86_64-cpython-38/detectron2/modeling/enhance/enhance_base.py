...
class enhance_base:
    def add_style(self, content, flag):
        ...
        style = self.load_style_img(self.cfg,wh=(content.size(3),content.size(2))).cuda()
        # else:
        ...
        #     else:
        #         style=self.load_style_img(wh=(content.size(3),content.size(2)))
        # content = torch.from_numpy(content).float().permute(2,0,1).cuda()
        
        with torch.no_grad():
            output = self.style_transfer(content[0], style, flag, self.alpha)
        for i in range(1, content.shape[0]):
    ...
    def __init__(self,  encoders, decoders, fcs, cfg):
        assert len(encoders) == len(decoders)
        self.pixel_means = np.array([[[103.530, 116.280, 123.675]]])
    ...
    def get_style_feats(self, ):
        feats = []
        feats.append(self.load_style_img(self.cfg).unsqueeze(0))
    ...
    def load_style_img(self,cfg, wh=None):
        # if self.args.random_style:
        if cfg.STYLE.FOGGY == False:
            with open(cfg.STYLE.ID, 'r', encoding='utf-8') as file:
        ...
        # 去掉每行末尾的换行符
            ...
            path = os.path.join(cfg.STYLE.PATH,random_line +'.jpg')  # 构造新的文件路径

        # list = os.listdir('/home/eason/eason/constractive_sfod/aa/dataset/clipart/JPEGImages')
    ...
    def show(self, feat, content=False,save=True):
        for i in range(feat.size(0)):
            ...
            if save:
                ...
                path = os.path.join(os.path.dirname(__file__), '..','/home/eason/AAAI/aa/TEST','noise')
