...
class enhance_vgg16(enhance_base):
    def __init__(self, cfg):
        ...
        fcs = self.get_fcs()
        print("using fcs...")
        vgg, decoder, fcs = self.load_param(cfg, vgg, decoder, fcs)
        self.encoders, self.decoders = self.splits(vgg,decoder)
        enhance_base.__init__(self, self.encoders, self.decoders, fcs, cfg)
    ...
    def load_param(self, cfg, vgg, decoder, fcs):
        ...
        for i in range(len(fcs)):
            for param in fcs[i].parameters():
                param.requires_grad = False
        decoder.load_state_dict(torch.load(cfg.STYLE.DECODER))
        vgg.load_state_dict(torch.load(cfg.STYLE.ENCODER)['model'])
        vgg = nn.Sequential(*list(vgg.children())[:19])
        fcs[0].load_state_dict(torch.load(cfg.STYLE.FC1))
        fcs[1].load_state_dict(torch.load(cfg.STYLE.FC2))
        print("loaded encoder: "+cfg.STYLE.ENCODER)
        print("loaded decoder: "+cfg.STYLE.DECODER)
        print("loaded fc1: "+cfg.STYLE.FC1)
        print("loaded fc2: "+cfg.STYLE.FC2)
