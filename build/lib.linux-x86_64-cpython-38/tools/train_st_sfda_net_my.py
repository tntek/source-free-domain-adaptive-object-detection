...
from detectron2.modeling.meta_arch.GCN import GCN
from detectron2.modeling.meta_arch.losses import gradient_discrepancy_loss
...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "cityscapes_instance":
        ...
        return CityscapesInstanceEvaluator(dataset_name, cfg)
...
def test_sfda(cfg, model):
    ...
    return results,results_i['bbox']['AP50']


def train_sfda(cfg, model_student, model_teacher, model_teacher_frozen=None):
    ...
    # model_teacher.eval()
    # result,ap50 = test_sfda(cfg, model_teacher)
    best_ap50 = 0
    ...
    with EventStorage(start_iter) as storage:
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in zip(data_loader, range(start_iter, max_iter)):
                ...
                # model_teacher_frozen.eval()
                with torch.no_grad():
                    _, teacher_features, teacher_proposals, teacher_results = model_teacher(data, mode="train")
                    # _, teacher_features, teacher_proposals, teacher_results = model_teacher_frozen(data, mode="train")


                teacher_pseudo_results_high, num_roih_proposal = process_pseudo_label(teacher_results, 0.9, "roih", "thresholding")
                if len(teacher_pseudo_results_high[0]._fields['gt_classes']) ==0:
                    continue
                move = iteration / (max_iter)
                loss_dict,cluster_acc,pse_acc = model_student(data, cfg, model_teacher, teacher_features, teacher_proposals, teacher_pseudo_results_high,epoch,args.cmaweight,args.emaweight,move,args.cluster_step,args.temp,estimate=True,mode="train")
                ...
                if iteration - start_iter > 5 and ((iteration + 1) % 50 == 0 or iteration == max_iter - 1):
                    ...
                    logger.info("epoch:{}, acc:{:.3f},pse_acc: {:.3f},base_acc:{:.3f} ,loss :{:}".format(epoch,a,b,c,['{0}: {1}'.format(k, v.item()) for k,v in loss_dict.items()]))
            ...
            a1/=j
            b1/=j
            logger.info("a1:{:.3f},b1:{:.3f},c1:{:.3f}".format(a1,b1,c1))

            model_student.eval()
            print("Student model testing@", epoch)
            result,ap50_s = test_sfda(cfg, model_student)
            ...
            if ap50 > best_ap50:
                torch.save(model_teacher.state_dict(), cfg.OUTPUT_DIR + "/best_model.pth".format(epoch))
                best_ap50 = ap50

            if ap50_s > best_ap50:
                torch.save(model_student.state_dict(), cfg.OUTPUT_DIR + "/best_model.pth".format(epoch))
                best_ap50 = ap50_s
          ...
          #  torch.save(model_student.state_dict(), cfg.OUTPUT_DIR + "/model_student_{}.pth".format(epoch))
            logger.info("best ap50:{:.3f}".format(best_ap50))
...
def setup(args):
    ...
    cfg.OUTPUT_DIR = cfg.OUTPUT_DIR + '/'+ args.method + '/' + time_string
    if args.method :
...
def main(args):
    ...
    # model_teacher_frozen = build_model(cfg)
    # DetectionCheckpointer(model_teacher_frozen, save_dir=cfg.OUTPUT_DIR).load(args.model_dir)
    # torch.set_rng_state(rng_state)
    # torch.cuda.set_rng_state(cuda_rng_state)

    return train_sfda(cfg, model_student, model_teacher,model_teacher_frozen=None)
