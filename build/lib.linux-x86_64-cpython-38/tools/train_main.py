...
def get_evaluator(cfg, dataset_name, output_folder=None):
    ...
    if evaluator_type == "bdd100k":
        return BddDetectionEvaluator(dataset_name,cfg)
    if evaluator_type == "kaist_viz":
        return KaistDetectionEvaluator(dataset_name,cfg)
    if evaluator_type == "kaist_tr":
        return KaistDetectionEvaluator(dataset_name,cfg)
    if evaluator_type == "kaist_viz_person":
        return KaistPersonDetectionEvaluator(dataset_name,cfg)
    if evaluator_type == "kaist_tr_person":
        return KaistPersonDetectionEvaluator(dataset_name,cfg)
    if evaluator_type == "flir":
        return FLIRDetectionEvaluator(dataset_name,cfg)
