    ...
    with EventStorage(start_iter) as storage:
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in zip(data_loader, range(start_iter, max_iter)):
                ...
                loss_dict = model_student(data, cfg, model_teacher, teacher_features, teacher_proposals, teacher_pseudo_results,teacher_pseudo_results_high,epoch,args.cmaweight,args.emaweight,move,args.cluster_step,args.temp,mode="train")
                ...
                if iteration - start_iter > 5 and ((iteration + 1) % 50 == 0 or iteration == max_iter - 1):
                    ...
                    logger.info("epoch:{},loss :{:}".format(epoch,['{0}: {1}'.format(k, v.item()) for k,v in loss_dict.items()]))
