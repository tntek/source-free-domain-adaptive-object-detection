...
def train_sfda(cfg, model_student, model_teacher, resume=False):
    ...
    # optimizer = optim.SGD(model_teacher.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0001)
    optimizer = build_optimizer(cfg, model_teacher)
    scheduler = build_lr_scheduler(cfg, optimizer)
    checkpointer = DetectionCheckpointer(model_teacher, cfg.OUTPUT_DIR, optimizer=optimizer, scheduler=scheduler)
    ...
    with EventStorage(start_iter) as storage:
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in zip(data_loader, range(start_iter, max_iter)):
                ...
                # loss_dict = model_student(data, cfg, model_teacher, teacher_features, teacher_proposals, teacher_pseudo_results,args.temp,epoch,mode="train")
            ...
            torch.save(model_teacher.state_dict(), cfg.OUTPUT_DIR + "/model_source_{}.pth".format(epoch))
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    cfg.merge_from_file('/home/eason/AAAI/aa/configs/sfda/foggy_baseline_vgg.yaml')
...
def main(args):
    ...
    DetectionCheckpointer(model_student, save_dir=cfg.OUTPUT_DIR).load("/home/eason/SFOD/vgg16_caffe.pth")
    DetectionCheckpointer(model_teacher, save_dir=cfg.OUTPUT_DIR).load("/home/eason/SFOD/vgg16_caffe.pth")
