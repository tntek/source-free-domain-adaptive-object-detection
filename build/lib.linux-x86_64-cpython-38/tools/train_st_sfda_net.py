    ...
    with EventStorage(start_iter) as storage:
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in zip(data_loader, range(start_iter, max_iter)):
                storage.iter = iteration + ( (epoch-1) * max_iter)
                optimizer.zero_grad()
                ...
                teacher_pseudo_results_low, num_roih_proposal = process_pseudo_label(teacher_results, 0.9, "roih", "thresholding")
                ...
                # if len(teacher_pseudo_results[0]._fields['gt_classes']) ==0:
                #     continue

                loss_dict = model_student(data, cfg, model_teacher, teacher_features, teacher_proposals, teacher_pseudo_results,teacher_pseudo_results_low,epoch,args.cmaweight,args.emaweight,move,args.cluster_step,args.temp,mode="train")

                # ,cluster_acc ,pse_acc,base_acc ,normalized_variance 

                # if iteration % args.temp ==0:
                #     cnn = cnn_f
                #     fc = fc_f
                # else:
                #     cnn = cnn
                ...
                # if warm == False and len(teacher_pseudo_results[0]._fields['gt_classes']) !=0 :
                #     loss_mt = loss_dict['loss_cls'].data + loss_dict['loss_box_reg'].data + loss_dict['loss_rpn_loc'] + loss_dict['loss_rpn_cls']
                #     loss_con = loss_dict['st_const'].data
                #     loss_cl = loss_dict['cont'].data

                #     storage.put_scalar("total_loss", losses.item(), smoothing_hint=False)
                #     storage.put_scalar("mt_loss", loss_mt, smoothing_hint=False)
                #     storage.put_scalar("con_loss", loss_con, smoothing_hint=False)
                #     storage.put_scalar("cl_loss", loss_cl, smoothing_hint=False)
                ...
                #     logger.info("epoch:{}, acc:{:.3f},pse_acc: {:.3f},base_acc:{:.3f} ,loss :{:} ,var :{:.3f}".format(epoch,a,b,c,['{0}: {1}'.format(k, v.item()) for k,v in loss_dict.items()],normalized_variance))
