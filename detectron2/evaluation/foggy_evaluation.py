...
import pdb
from .draw_TP_FP_FN import count
...
class FoggyDetectionEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        if self.cfg.TEST.SAVE_DETECTION_PKLS:
            import pickle
            path = os.path.join(self.cfg.OUTPUT_DIR,'detections.pckl')
            f = open(path,'wb')
            pickle.dump([predictions,self._image_set_path,self._anno_file_template,self._class_names],f)
            f.close()

            all_TPS,all_FPS,all_FNS,fnr = count(path, 0.5, 'jpg')

            self._logger.info("TP: {}, FP: {}, FN: {}, FNR: {}".format(all_TPS,all_FPS,all_FNS,fnr))
            if os.path.exists(path):
                os.remove(path)
                print(f"Deleted file after use: {path}")
...
# 在 class PascalVOCDetectionEvaluator 的 evaluate 方法中
                ...
                results= os.path.join("/home/eason/AAAI/aa/martix", "{}.txt")
